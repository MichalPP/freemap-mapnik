...
function generateFreemapStyle(shading = shadingCfg, contours = contoursCfg, hikingTrails = hikingTrailsCfg, bicycleTrails = bicycleTrailsCfg) {
  return createMap({
    ...
    .style('aeroways')
      .rule({ minZoom: 11 })
        .lineSymbolizer({ stroke: '#BDB76B', strokeOpacity: 0.7, strokeWidth: 4, strokeDasharray: '10,2' })
    .style('highways')
    ...
    .style('infopoints')
      .poiIcons(infoPois)
    .style('transport_points')
      .typesRule(11, 'aerodrome')
        .markersSymbolizer({ file: 'images/aerodrome.svg', width: 12, height: 12, fill: '#000000' })
